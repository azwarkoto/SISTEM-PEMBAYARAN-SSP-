...
include ("lib/koneksi.php");	
	...
	if($_POST['submit']){
		...
		$cek 			= mysql_query("SELECT password FROM siswa WHERE password='$password_lama'");
		...
		if($qry){
			...
			if(strlen($password_baru) >= 5){
				...
				if($password_baru == $konfirmasi_password){
					...
					$user 		= $_SESSION['nis_murid']; //ini dari session saat login
					
					$update 		= mysql_query("UPDATE siswa SET password='$password_baru' WHERE nis_murid='$user'");
					if($update){
						//kondisi jika proses query UPDATE berhasil
						echo "<script>alert('Password Berhasil di Rubah!');location.href='siswa.php';</script>";
					}else{
...
<center><h2>Form Ganti Password</h2></center>
	<form method="post" action="">
		<table 	width="53%" border="0" align="center">
			<tr>
				<td>Nis Murid</td>
				<td><?php echo $_SESSION['nis_murid'];?></td>
			<tr>
			<tr>
				<td>Password Lama</td>
				<td>:</td>
